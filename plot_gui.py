...
class AxisDialog(Dialog):
    def body(self, master):
        tk.Label(master, text="最小值：").grid(row=0, column=0, sticky="e")
        tk.Label(master, text="最大值：").grid(row=1, column=0, sticky="e")
        tk.Label(master, text="分段数：").grid(row=2, column=0, sticky="e")
...
def enable_xaxis_editing(fig, ax, master):
    ...
    def on_pick(event):
        ...
        if not (isinstance(artist, Text) and artist in ax.get_xticklabels()):
            return
        dialog = AxisDialog(master, title = "自定义x轴")
...
def plot_cum_distribution():
    ...
    for col in data_cols:
        try:
            dic = plot.cum_prob(
        plot.radix_sort_with_negative(plot.filter(col), round_input)
        )
            data_dicts.append(dic)
        except ValueError:
            messagebox.showerror("不正确的输入",
                                  "无法计算Z值。请确认：\n\n1.每一列都至少有2个数据\n\n2.四舍五入之后每一列数据至少有一对不同的数字")
    ...
    plot.plot_distribution(data_dicts,
                            new_figure=new_fig,
...
def plot_z_value():
    ...
    for col in data_cols:
        ...
        plot.radix_sort_with_negative(plot.filter(col), round_input))
            ...
            messagebox.showerror("不正确的输入", "无法计算Z值。请确认：\n\n1.每一列都至少有2个数据\n\n2.四舍五入之后每一列数据至少有一对不同的数字")
...
chk_grid = tk.Checkbutton(container, text = "添加网格",
