...
def _refresh_ref_legend(ax, fig):
    if not hasattr(ax, "_ref_lines"):
        ax._ref_lines = []
    if getattr(ax, "_ref_legend", None):
        try:
    ...
    if handles:
        ...
        # 参考线图例不参与编辑/拾取
        for t in ax._ref_legend.get_texts():
            t.set_picker(False)
            t._no_edit = True

    # 关键：主图例只挂一次，并清理历史重复
    _ensure_main_legend_once(ax)
    fig.canvas.draw_idle()


def _ensure_main_legend_once(ax):
    leg = getattr(ax, "_main_legend", None)
    if not leg:
        return
    arts = list(getattr(ax, "artists", []))

    found = False
    for a in arts:
        if a is leg:
            if found:
                # 重复的同一 legend，直接从 Axes 中移除
                try:
                    a.remove()
                except Exception:
                    pass
...
#将读取到的数据进行绘图
    ...
    if new_figure:
        ...
        if getattr(ax, "_ref_legend", None) and ax._ref_legend not in ax.artists:
